...
class TransactionFactory {
  ...
  middleware(...providerNames) {
    ...
    return async function (ctx, next) {
      const closers = [];
      ctx.transaction = await self[sOpenConnections](providerNames, closers); // try awaiting on next middleware and committing statements afterwards. Rollback if there was an error
...
TransactionFactory.prototype[sOpenConnections] = function (providerNames, closers) {
  ...
  return new Promise((resolve, reject) => {
    ...
    if (providers.length === 0) {
      this[sRavelInstance].$log.debug('Middleware transaction attempted, but no database providers are registered.'); // resolve with no connections

      resolve(Object.create(null));
    } else {
      const sConnName = Symbol.for('name');
      const toOpen = providerNames.length === 0 ? providers : providers.filter(p => providerNames.indexOf(p.name) >= 0); // index provider promises in an array and use co to open connections.

      Promise.all(toOpen.map(p => {
        return p.getTransactionConnection() // chain an extra then() on the end, which will store connection closing functions
