    ...
    static credentialToProfile(token, client) {
      return new Promise((resolve, reject) => {
        ...
        if (provider.length > 0) {
          ...
          ravelInstance.$kvstore.get(profileCacheKey, function (err, profileString) {
            if (err || !profileString) {
              provider[0].credentialToProfile(token, client).then(result => {
          ...
          reject(new ravelInstance.$err.NotFound('No registered authentication providers handle client type ' + client), null);
        }
      });
    }
