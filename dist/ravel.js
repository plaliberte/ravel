...
class Ravel extends AsyncEventEmitter {
  ...
  constructor() {
    ...
    this[sKeygripKeys] = []; // how we store modules for dependency injection

    this[coreSymbols.modules] = Object.create(null); // how we store routes for later access

    this[coreSymbols.routes] = Object.create(null); // how we store resources for later access

    this[coreSymbols.resource] = Object.create(null); // how we store middleware for dependency injection

    this[coreSymbols.middleware] = Object.create(null); // current working directory of the app using the
    ...
    this[coreSymbols.params] = Object.create(null); // a list of registered module factories which haven't yet been instantiated

    this[coreSymbols.moduleFactories] = Object.create(null);
    this[coreSymbols.resourceFactories] = Object.create(null);
    this[coreSymbols.routesFactories] = Object.create(null); // Allows us to detect duplicate binds

    this[coreSymbols.endpoints] = new Map(); // a list of known modules, resources and routes so that metadata can be retrieved from them

    this[coreSymbols.knownComponents] = Object.create(null); // init errors

    this[sErr] = require('./util/application_error'); // init logging system

    this[sLog] = new (require('./util/log'))(this); // init dependency injection utility, which is used by most everything else

    this[coreSymbols.injector] = new (require('./core/injector'))(this, module.parent !== null ? module.parent : module); // Register known ravel parameters
    ...
    this.registerParameter('redis websocket channel prefix', true, 'ravel.ws'); // redis parameters
    ...
    this.registerParameter('redis keepalive interval', true, 1000); // Node/koa parameters

    this.registerParameter('port', true, 8080);
    this.registerParameter('https', true, false); // supports any option from https://nodejs.org/api/tls.html#tls_tls_createserver_options_secureconnectionlistener

    this.registerParameter('https options', true, {});
    this.registerParameter('public directory');
    this.registerParameter('favicon path');
    this.registerParameter('keygrip keys', true); // session parameters
    ...
    this.registerParameter('session rolling', true, false); // Passport parameters
  ...
  async init() {
    ...
    await this.emit('pre init'); // log uncaught errors to prevent ES6 promise error swallowing
    ...
    process.on('unhandledRejection', err => {
      ...
      if (this[sUncaughtRejections] >= 10) {
        ...
        this.$log.error(`Encountered ${this[sUncaughtRejections]} or more uncaught rejections. Re-run ` + 'node and ravel with full logging output for more information:' + '\n- node --trace-warnings app.js' + '\n- app.set(\'log level\', app.$log.TRACE);');
      } else {
    ...
    this.$kvstore = require('./util/kvstore')(this); // App dependencies.
    ...
    const router = new (require('./core/router'))(); // configure koa

    const app = new Koa();
    app.proxy = true; // the first piece of middleware is the exception handler
    // catch all errors and return appropriate error codes
    // to the client

    app.use(new Rest(this).errorHandler()); // enable gzip compression

    app.use(compression()); // configure redis session store

    this[sKeygripKeys] = [...this.get('keygrip keys')];
    this[sKeygrip] = require('keygrip')(this[sKeygripKeys], 'sha256', 'base64');
    app.keys = this[sKeygrip]; // configure session options

    if (!this.get('https') && !this.get('session secure')) {
      this.$log.warn("app.set('session secure', false) results in insecure sessions" + ' over HTTP and introduces critical security vulnerabilities.' + ' This is intended for development purposes only, or for use' + ' behind a TLS termination proxy.');
    }
    ...
    const sessionOptions = {
      ...
      secure: this.get('https') || this.get('session secure'),

      /* (boolean) secure or not (default true) */
      rolling: this.get('session rolling')
    ...
    app.use(session(sessionOptions, app)); // favicon
    ...
    require('./auth/passport_init.js')(this, router); // create websocket broker


    const WebsocketBroker = require('./core/websocket.js');

    this[coreSymbols.websocketBroker] = new WebsocketBroker(this); // basic koa configuration is completed

    await this.emit('post config koa', app); // create registered modules using factories

    await this[coreSymbols.moduleInit]();
    await this.emit('post module init');
    await this.emit('pre routes init', app); // create registered resources using factories

    this[coreSymbols.resourceInit](router); // create routes using factories

    this[coreSymbols.routesInit](router); // include routes as middleware

    app.use(router.middleware()); // Create koa callback and server

    this[sCallback] = app.callback();
    this[sServer] = this.get('https') ? https.createServer(this.get('https options'), this[sCallback]) : http.createServer(this[sCallback]); // application configuration is completed
  ...
  async listen() {
    if (!this[sInitialized]) {
      ...
      return new Promise((resolve, reject) => {
        // validate parameters
        this[coreSymbols.validateParameters](); // Start Koa server
