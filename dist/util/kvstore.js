...
function retryStrategy(ravelInstance) {
  return function (options) {
    ...
    if (code === 'ECONNREFUSED') {
      ...
      ravelInstance.$log.error(`Lost connection to redis: ${code}. Max retry attempts exceeded.`); // End reconnecting with built in error

      return new $err.General(`Lost connection to redis: ${code}. Max retry attempts reached.`);
    } else {
      const time = Math.pow(options.attempt, 2) * 100;
      ravelInstance.$log.error(`Lost connection to redis: ${code}. Reconnecting in ${time} milliseconds.`); // reconnect after
...
function disable(client, fn) {
  client[fn] = function () {
    throw new $err.General(`kvstore cannot use ${fn}(). Use.$kvstore.clone() to retrieve a fresh connection first.`);
  };
}
/**
...
function createClient(ravelInstance, restrict = true) {
  ...
  ravelInstance.on('post init', () => {
    ravelInstance.$log.info(localRedis ? 'Using in-memory key-value store. Please do not scale this app horizontally.' : `Using redis at ${ravelInstance.get('redis host')}:${ravelInstance.get('redis port')}`);
  });
  let client;
  ...
  if (localRedis) {
    ...
    client = redis.createClient(ravelInstance.get('redis port'), ravelInstance.get('redis host'), {
      no_ready_check: true,
