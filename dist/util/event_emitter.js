...
class AsyncEventEmitter {
  ...
  emit(eventName, ...args) {
    if (!this[sEventMap].has(eventName)) {
      ...
      const toExecute = this[sEventMap].get(eventName); // remove onces immediately, before invoking

      this[sEventMap].set(eventName, toExecute.filter(l => !l.once)); // invoke (mapping to promises)
