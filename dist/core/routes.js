...
const coreServices = require('./services'); // Symbols for HTTP methods
...
function buildRoute(ravelInstance, routes, koaRouter, methodName, meta) {
  ...
  let args = [fullPath]; // build middleware from metadata

  const middleware = []; // apply class-level @authenticated middleware, if present
  ...
  if (Metadata.getClassMeta(routes, '@authenticated')) {
    ...
    middleware.push(new AuthenticationMiddleware(ravelInstance, config.redirect, config.allowRegistration).middleware());
  ...
  middleware.push(rest.respond()); // apply class-level and method-level @transaction middleware, if present (in the correct order)
  ...
  let toInject = [].concat(Metadata.getClassMetaValue(routes, '@before', 'middleware', [])); // then method-level @before middleware, if any
  ...
  for (let i = 0; i < toInject.length; i++) {
    ...
    if (typeof middlewareFactory === 'object' && middlewareFactory.isFactory) {
      if (i + middlewareFactory.fn.length >= toInject.length) {
        throw new ravelInstance.$err.IllegalValueError(`Middleware ${m} requires ${middlewareFactory.fn.length} arguments for instantiation.
  ...
  if (Metadata.getMethodMeta(routes, methodName, '@cache')) {
    ...
    middleware.push(new ResponseCacheMiddleware(ravelInstance).middleware(config));
  ...
  if (meta.endpoint) {
    middleware.push(async function (ctx, next) {
      const result = meta.endpoint.bind(routes)(ctx); // if handler returns a Promise, await on it
  ...
  args = args.concat(middleware); // now call underlying koa method to register middleware at specific route
...
function initRoutes(ravelInstance, koaRouter) {
  const proto = Object.getPrototypeOf(this); // handle class-level @mapping decorators
...
module.exports = function (Ravel) {
  ...
  Ravel.prototype[symbols.loadRoutes] = function (routesClass) {
    const basePath = Metadata.getClassMetaValue(routesClass.prototype, '@role', 'name'); // if routes with this base path has already been registered, error out
    ...
    Metadata.putClassMeta(routesClass.prototype, 'ravel', 'instance', this); // store known routes module with path as the key, so someone can reflect on the class

    this[symbols.registerClassFunc](basePath, routesClass); // build routes instantiation function, which takes the
