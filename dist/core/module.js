...
async function initModule(ravelInstance) {
  const self = this;
  const name = Metadata.getClassMetaValue(Object.getPrototypeOf(self), '@role', 'name'); // run @postinject handlers

  const postInjects = Metadata.getClassMeta(Object.getPrototypeOf(self), '@postinject', Object.create(null));
  await Promise.all(Object.keys(postInjects).map(f => postInjects[f].call(self))); // connect any Ravel lifecycle handlers to the appropriate events
  ...
  connectHandlers('@preclose', 'end'); // connect @interval
  ...
  ravelInstance.once('post module init', () => {
    for (const m of Object.keys(middleware)) {
      if (ravelInstance[symbols.moduleFactories][m]) {
        throw new $err.DuplicateEntry(`Unable to register @middleware with name ${m}, which conflicts with an existing Module name`);
      } else {
...
module.exports = function (Ravel) {
  ...
  Ravel.prototype[symbols.loadModule] = function (moduleClass) {
    ...
    if (this[symbols.moduleFactories][name]) {
      ...
      throw new this.$err.DuplicateEntry(`Module with name '${name}' has already been registered.`);
    } // store reference to this ravel instance in metadata


    Metadata.putClassMeta(moduleClass.prototype, 'ravel', 'instance', this); // store name in metadata

    Metadata.putClassMeta(moduleClass.prototype, 'source', 'name', name); // store known module with path as the key, so someone can reflect on the class

    this[symbols.registerClassFunc](name, moduleClass); // build injection function
    ...
    this[symbols.moduleFactories][name] = async () => {
      ...
      await initModule.call(temp, this); // overwrite uninitialized module with the correct one
    ...
    this[symbols.moduleFactories][name].children = []; // save uninitialized module to Ravel.modules
  ...
  Ravel.prototype[symbols.moduleInit] = async function () {
    const rootFactories = Object.create(null); // build dependency graph
    ...
    for (const moduleName of Object.keys(this[symbols.moduleFactories])) {
      ...
      this[symbols.moduleFactories][moduleName].parents = factoryDeps; // If this module has no dependencies on other client module factories,
    ...
    const calcDepth = (moduleFactory, visitedTag, startModule, last) => {
      ...
      if (visitedMeta.get(moduleFactory) === visitedTag) {
        throw new this.$err.General(`Module instantiation failed. A cyclical dependency exists between modules ${startModule} and ${last}`);
        ...
        for (const p of moduleFactory.parents) {
          ...
          const pDepth = p.maxDepth !== undefined ? p.maxDepth : calcDepth(p, visitedTag, startModule, moduleFactory.moduleName);
