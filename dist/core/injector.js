...
class Injector {
    ...
    if (moduleMap[moduleName] !== undefined) {
      ...
      try {
        ...
        if (e.message.indexOf('Cannot find module') >= 0) {
          throw new this[sRavelInstance].$err.NotFound('Unable to inject ' + 'requested module \'' + moduleName + '\'. If it is ' + 'one of your modules, make sure you register it with ' + 'Ravel.module before running Ravel.start. If it is an NPM ' + 'dependency, make sure it is in your package.json and that it ' + 'has been installed via $ npm install.');
        } else {
  ...
  inject(moduleMap, DIClass) {
    ...
    const instance = new DIClass(...args); // then do @autoinject
