...
module.exports = function (Ravel) {
  ...
  Ravel.prototype[symbols.loadParameters] = function () {
    ...
    do {
      currentDir = nextDir; // load parameters from config file into an empty object
    ...
    } while (nextDir !== currentDir); // verify all parameters in fromFile are known


    for (const key of Object.keys(fromFile)) {
      if (!this[symbols.knownParameters][key]) {
        throw new this.$err.IllegalValue(`Attempted to set unknown parameter: ${key.toString()}`);
      }
    }
    ...
    Object.assign(defaults, fromFile); // now merge with this[symbols.params], allowing programmatically set params to take precedence

    Object.assign(defaults, this[symbols.params]); // now defaults contains what we want, so make it this[symbols.params]

    this[symbols.params] = defaults; // done!
  ...
  Ravel.prototype[symbols.validateParameters] = function () {
    const unknowns = Object.keys(this[symbols.knownParameters]).filter(p => this[symbols.knownParameters][p].required).filter(p => {
      try {
        this.get(p);
        return false;
      } catch (err) {
    ...
    if (unknowns.length > 0) {
      throw new this.$err.NotFound(`Required parameters have not been defined yet:\n${unknowns.join('\n')}`);
    }
  };
  ...
  Ravel.prototype.get = function (key) {
    if (!this[symbols.parametersLoaded]) {
      ...
      throw new this.$err.NotFound(`Known required parameter ${key} was requested, but hasn't been defined yet.`);
  ...
  Object.defineProperty(Ravel.prototype, 'config', {
    get: function () {
      return JSON.parse(JSON.stringify(this[symbols.params]));
    }
  });
};